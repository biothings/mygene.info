...
class TestQueryGET(BiothingsDataTest):
    ...
    def test_209_query(self):
        ...
        query_data = {"q": "U3", "scopes": "symbol, alias", "entrezonly": "True"}
    ...
    def test_222_facet(self):
        res_t = self.request(
            "query?q=symbol:cdk?&facets=type_of_gene&species=human,mouse,rat"
        ).json()
    ...
    def test_223_facet(self):
        ...
        assert [
            x["count"] for x in res2["facets"]["taxid"]["terms"] if x["term"] == 9606
        ][0] == res2["total"]
...
class TestQueryPOST(BiothingsDataTest):
    ...
    def test_264_post(self):
        data = {"q": '[1017, "1018"]', "scopes": "entrezgene", "jsoninput": "true"}
        res = self.request("query", method="POST", data=data).json()
        assert len(res) == 2
        assert res[0]["_id"] == "1017"
        assert res[1]["_id"] == "1018"
