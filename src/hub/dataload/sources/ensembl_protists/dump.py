...
class EnsemblProtistsBioMart(GenericBioMart):
    ...
    def get_latest_mart_version(self):
        ...
        ftp.login()
        release_li = [x for x in ftp.nlst('/pub/protists') if x.startswith('/pub/protists/release-')]
    ...
    def select_species(self):
        ...
        try:
            ...
            ftp = FTP(self.__class__.ENSEMBL_FTP_HOST)
            ftp.login()
            species_file = '/pub/protists/release-%s/mysql/protists_mart_%s/dataset_names.txt.gz' % (self.release, self.release)
