...
extra_mapping_lock = multiprocessing.get_context("fork").Lock()
...
class EnsemblParser(object):
    ...
    def _load_ensembl2entrez_li(self, src_name):
        ...
        ensembl2entrez = tab2dict(datafile, (1, 2), 0, includefn=_not_LRG, alwayslist=True)
            ...
            for id, doc in datadict.items():
                if id.isdigit():
                    if skip_count < ERR_THRESHOLD:
                        ...
                        raise ValueError("Too many ensembl ids are entirely numeric")
                    self.logger.warning("Document Skipped: All-digit id {}".format(id))
                    continue
                doc["_id"] = id
                yield doc
    ...
    def load_ensembl2pos(self):
        ...
        ensembl2pos = value_convert(
            ensembl2pos,
            lambda x: {"ensemblgene": x[0], "chr": x[3], "start": int(x[1]), "end": int(x[2]), "strand": int(x[4])},
        )
        ...
        for datadict in tab2dict_iter(datafile, (1, 1, 3, 4, 5, 6), 0, includefn=_not_LRG):
            ...
            datadict = value_convert(
                datadict, lambda x: {"genomic_pos": x, "__aslistofdict__": "genomic_pos"}, traverse_list=False
            )
            for doc in map_id(datadict, self.ensembl2entrez):
                yield doc
    ...
    def load_ensembl2interpro(self):
        ...
        for datadict in tab2dict_iter(datafile, (1, 4, 5, 6), 0):
            ...
            datadict = value_convert(
                datadict, lambda x: {"interpro": x, "__aslistofdict__": "interpro"}, traverse_list=False
            )
            for doc in map_id(datadict, self.ensembl2entrez):
                yield doc
