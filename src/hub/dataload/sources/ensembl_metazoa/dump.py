...
class EnsemblMetazoaBioMart(GenericBioMart):
    ...
    def get_latest_mart_version(self):
        ...
        ftp.login()
        release_li = [x for x in ftp.nlst('/pub/metazoa') if x.startswith('/pub/metazoa/release-')]
    ...
    def select_species(self):
        ...
        try:
            ...
            ftp = FTP(self.__class__.ENSEMBL_FTP_HOST)
            ftp.login()
            species_file = '/pub/metazoa/release-%s/mysql/metazoa_mart_%s/dataset_names.txt.gz' % (self.release, self.release)
