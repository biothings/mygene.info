...
class MyGeneHubServer(HubServer):
    ...
    def configure_commands(self):
        ...
        self.commands["publish_diff_demo"] = partial(self.managers["release_manager"].publish_diff,config.S3_APP_FOLDER + "-demo",
                                                s3_bucket=config.S3_DIFF_BUCKET + "-demo")
        self.commands["snapshot_demo"] = partial(self.managers["snapshot_manager"].snapshot,repository=config.SNAPSHOT_REPOSITORY + "-demo")
        self.commands["publish_snapshot_demo"] = partial(self.managers["release_manager"].publish_snapshot,s3_folder=config.S3_APP_FOLDER + "-demo",
                                            repository=config.READONLY_SNAPSHOT_REPOSITORY)
        # replace default
        self.commands["publish_diff_prod"] = partial(self.managers["release_manager"].publish_diff,config.S3_APP_FOLDER)
        self.commands["publish_snapshot_prod"] = partial(self.managers["release_manager"].publish_snapshot,s3_folder=config.S3_APP_FOLDER)
